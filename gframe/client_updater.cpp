...
void ClientUpdater::Unzip(void* payload, unzip_callback callback) {
	...
	for(const auto& file : update_urls) {
		uzpl.cur = i++;
		const auto name = epro::format(UPDATES_FOLDER, ygo::Utils::ToPathString(file.name));
		uzpl.filename = name.data();
		if(!Utils::UnzipArchive(name, callback, &cbpayload)) {
			ErrorLog("[Realm Updater] FAILED: Could not extract update archive: {}", file.name);
			failed = true;
			break;
		}
	}
	...
	if(failed || !Utils::FileExists(path)) {
		...
		ErrorLog("[Realm Updater] FAILED: Update not installed; restored the previous executable.");
		failed = true;
		return;
	}
#endif
