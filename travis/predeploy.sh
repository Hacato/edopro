...
# Publish the Realm bootstrap launcher when the Windows workflow built it.
# This file is intentionally separate from the small automatic engine-update ZIP.
if [[ "$PLATFORM" == "windows" ]] && [[ -f "Realm of Kings.exe" ]]; then
    cp -f "Realm of Kings.exe" "deploy/Realm of Kings.exe"
fi
...
EOF
        ...
        if [[ "$COMPLETE_OK" == true ]]; then
            ...
            if [[ -f "$PWD/deploy/EDOPro.exe" ]]; then
                ...
                if [[ -f "$PWD/deploy/Realm of Kings.exe" ]]; then
                    cp -f \
                        "$PWD/deploy/Realm of Kings.exe" \
                        "$COMPLETE_DIR/Realm of Kings.exe"
                else
                    echo
                    echo "REALM COMPLETE CLIENT WARNING:"
                    echo "deploy/Realm of Kings.exe was not found."
                    echo
                    COMPLETE_OK=false
                fi

                echo
                echo "REALM COMPLETE CLIENT: Realm engine and launcher installed"
                echo
        ...
        if [[ "$COMPLETE_OK" == true ]]; then
            ...
            echo
            echo "A new player can extract this ZIP and launch Realm of Kings.exe."
            echo "============================================================"
            echo
